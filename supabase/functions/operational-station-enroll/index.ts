...
function createEnrollAdminClient(supabaseUrl: string, serviceKey: string) {
  return createClient(supabaseUrl, serviceKey, {
    auth: { persistSession: false, autoRefreshToken: false }
  })
}

type EnrollAdminClient = ReturnType<typeof createEnrollAdminClient>

async function removeAutogeneratedTechnicalProfile(
  adminClient: EnrollAdminClient,
  authUserId: string,
  deviceId: string
): Promise<{ ok: true } | { ok: false; reason: string }> {
  const { data: userRow, error: getErr } = await adminClient.auth.admin.getUserById(authUserId)
  if (getErr || !userRow.user) return { ok: false, reason: "auth_user_missing" }
  const authUser = userRow.user
  const isTechnical =
    authUser.app_metadata?.operational_station_device === true
    || authUser.user_metadata?.operational_station_device === true
  const metaDeviceId = String(
    authUser.app_metadata?.device_id || authUser.user_metadata?.device_id || ""
  )
  if (!isTechnical || metaDeviceId !== deviceId) {
    return { ok: false, reason: "technical_identity_mismatch" }
  }
  const { error: delErr } = await adminClient.from("profiles").delete().eq("id", authUserId)
  if (delErr) return { ok: false, reason: "profile_delete_failed" }
  return { ok: true }
}
...
Deno.serve(async (req) => {
  ...
  const adminClient = createEnrollAdminClient(supabaseUrl, serviceKey)
  ...
  if (action === "complete") {
    ...
    try {
      const { data: created, error: createError } = await adminClient.auth.admin.createUser({
        ...
        password,
        email_confirm: true,
        app_metadata: { operational_station_device: true, device_id: deviceId },
        user_metadata: { device_id: deviceId, operational_station_device: true }
      })
      ...
      const profileCleanup = await removeAutogeneratedTechnicalProfile(
        adminClient,
        authUserId,
        deviceId
      )
      if (!profileCleanup.ok) {
        await adminClient.auth.admin.deleteUser(authUserId).catch(() => {})
        authUserId = null
        await adminClient.rpc("fail_station_device_enrollment", {
          p_device_id: deviceId,
          p_enrollment_id: enrollmentId,
          p_reason: `profile_cleanup_${profileCleanup.reason}`
        })
        return genericInvalid(allowedOrigin)
      }
