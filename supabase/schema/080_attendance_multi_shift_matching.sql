-- Multi-shift attendance matching: assign clock-in/out per schedule block (not per day).
-- Sanitize technical metadata from exported observations.
-- Apply after 071_fix_late_attendance.sql.

create or replace function public.sanitize_attendance_observation(p_text text)
returns text
language sql
immutable
as $$
  select nullif(trim(
    regexp_replace(
      ...
      '(^|\s)(device_ua|user_agent|session_id|raw_metadata|ip_address)\s*[:=][^\n]*',
      '',
      'gi'
    )
  ), '');
...
revoke all on function public.sanitize_attendance_observation(text) from public;
grant execute on function public.sanitize_attendance_observation(text) to authenticated;
