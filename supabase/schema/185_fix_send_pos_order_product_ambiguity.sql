-- Fix ERROR 42702: column reference "product.id" is ambiguous in send_pos_order_to_production.
-- Root cause: migration 157 introduced PL/pgSQL variable `product` plus SQL alias `product`
-- on public.pos_products in inventory loops (~lines 718 and 807 in 157 prosrc).
-- Migration 158 fixed this in repo but was not applied to preprod before F0A Sim B.
-- Apply after 184_pos_order_owner_f0a.sql (or latest schema).
--
-- Fix strategy (same as 158, re-issued as 185 for post-184 traceability):
--   - Prefix PL/pgSQL record variables with v_ (v_product, v_pos_order, …)
--   - Use table alias `pop` / `poi` / `ar` in SQL (never `product` as alias)
-- Preserves: signature, security definer, search_path, grants, KDS, inventory, is_test_item, events.
