...
begin
  ...
  where c.pipeline_status = 'discarded'
    and public.recruitment_candidate_discarded_at(c.id) is not null
    and public.recruitment_candidate_discarded_at(c.id) < now() - make_interval(days => p_retention_days)
    and not exists (
      select 1
...
begin
  ...
  perform public.purge_stale_discarded_recruitment_candidates(60);
