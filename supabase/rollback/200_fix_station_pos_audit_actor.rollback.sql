-- Rollback 200_fix_station_pos_audit_actor.sql
-- Restores audit_pos_order_change() to auth.uid() attribution (pre-200).
-- Does NOT drop pos_order_event_actor_profile (harmless if left).

begin;

drop function if exists public.pos_order_event_actor_profile(uuid, uuid, uuid);
