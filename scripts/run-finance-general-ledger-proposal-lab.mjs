/**
 * Disposable local lab for Libro Mayor migration 215.
 * Applies the finance schema through 204, then 208, 209 and 215.
 * Does not connect to Stage or Production and does not pull images.
 *
 * Usage: node scripts/run-finance-general-ledger-proposal-lab.mjs
 */
...
const migration = join(schemaDir, "215_finance_general_ledger.sql")
const migrationTest = join(schemaDir, "215_test_finance_general_ledger.sql")
const rollback = join(rollbackDir, "215_finance_general_ledger.rollback.sql")
const container = `finance-ledger-215-${Date.now()}`
const port = 55490 + Math.floor(Math.random() * 20)
...
const requiredScenarios = [
  ...
  "17_dimensions_affect_opening_and_closing",
  "18_search_keeps_real_balance",
  "19_page_running_balance",
  "20_deterministic_order",
...
try {
  ...
  psqlFile(migration, "apply_215")
  assertScenarios(psqlFile(migrationTest, "test_215_first", ["-At", "-F", "|"]), "first")

  psqlFile(rollback, "rollback_215")
  psqlFile(rollback, "rollback_215_again")
  ...
  if (verifyLine !== "0|1|true|false|false") {
    throw new Error(`Rollback verification failed: ${verifyLine || afterRollback}`)
  }

  psqlFile(migration, "reapply_215")
  assertScenarios(psqlFile(migrationTest, "test_215_second", ["-At", "-F", "|"]), "second")
  console.log("215 apply, test, rollback, reapply, retest complete")
} catch (error) {
